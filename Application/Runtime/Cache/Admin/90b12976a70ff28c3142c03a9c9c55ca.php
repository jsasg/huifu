...
<body>
	...
	<div class="main-title">
		...
		<div class="title-button floatR">
			<a href="<?php echo U('Column/add');?>" class="btn btn-danger">
			...
			<table class="table margin-bottom-0">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if($vo['is_footer'] == 1): ?>是
								<?php else: ?>
									否<?php endif; ?>
							</td>
							<td>
								<a href="<?php echo U('Column/add',array('id'=>$vo['id']));?>">
									编辑
								</a>|
								<a href="<?php echo U('Column/delete',array('id'=>$vo['id']));?>">
									删除
								</a>
							</td>
						</tr>
						<?php if(is_array($vo['subNode'])): $i = 0; $__LIST__ = $vo['subNode'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><tr>
								<td></td>
								<td></td>
								<td>&emsp;&emsp;|—<?php echo ($sub['column_name']); ?></td>
								<td><?php echo ($sub['column_sort']); ?></td>
								<td>
									<?php if($sub['is_open'] == 1): ?>是
									<?php else: ?>
										否<?php endif; ?>
								</td>
								<td>
									<?php if($sub['is_footer'] == 1): ?>是
									<?php else: ?>
										否<?php endif; ?>
								</td>
								<td>
									<a href="<?php echo U('Column/add',array('id'=>$sub['id']));?>">
										编辑
									</a>|
									<a href="<?php echo U('Column/delete',array('id'=>$sub['id']));?>">
										删除
									</a>
								</td>
