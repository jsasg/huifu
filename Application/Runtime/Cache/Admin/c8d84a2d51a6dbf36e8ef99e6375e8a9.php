...
<body>
	...
	<div class="main-content">
		<form action="<?php echo U('activity/delete');?>" method="get" id="deleterForm">
			...
			<table class="table margin-bottom-0">
				<thead>
					<tr class="active">
						...
						<th>
                            <span>新增时间</span>
                            <?php if(($_GET['sort_time'] == 'up') || ($_GET['sort_time'] == false)): ?><a href="<?php echo U('Activity/index',array('sort_time'=>'down'));?>">
                            <?php else: ?>
                                <a href="<?php echo U('Activity/index',array('sort_time'=>'up'));?>"><?php endif; ?>
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo['sign_up_num']); ?></td>
							<td><?php echo (date('Y-m-d',(isset($vo['sign_up_end']) && ($vo['sign_up_end'] !== ""))?($vo['sign_up_end']):'')); ?></td>
							<td><?php echo (date("Y-m-d",(isset($vo['start_time']) && ($vo['start_time'] !== ""))?($vo['start_time']):'')); ?></td>
							<td><?php echo (date("Y-m-d",(isset($vo['end_time']) && ($vo['end_time'] !== ""))?($vo['end_time']):'')); ?></td>
							<td>
							...
							<td>
								<a href="<?php echo U('activity/apply',array('id'=>$vo['id']));?>">
									查看报名列表
								</a>|
								<a href="<?php echo U('activity/add',array('id'=>$vo['id']));?>">
									编辑
								</a>|
								<a href="<?php echo U('activity/delete',array('id'=>$vo['id']));?>">
									删除
								</a>
							</td>
