...
<body>
	...
	<div class="main-title">
		<h4 class="title-text floatL">节点管理</h4>
		<div class="title-button floatR">
			<a href="<?php echo U('Node/insert');?>" class="btn btn-danger">
	...
	<div class="main-content">
		<form action="<?php echo U('Node/delete');?>" method="get" id="deleterForm">
		<div class="margin-top-10">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="cate-item p-node">
                    ...
                    <div class="cate-action text-right">
                        <a href="<?php echo U('Node/insert',array('id'=>$vo['id']));?>">
                            编辑
                        </a>|
                        <a href="<?php echo U('Node/delete',array('id'=>$vo['id']));?>">
                            删除
                        </a>
                    </div>
                </div>
                <?php if(is_array($vo[sub])): $i = 0; $__LIST__ = $vo[sub];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$su): $mod = ($i % 2 );++$i;?><div class="su display-none">
                        <div class="cate-item">
                            <div class="cate-text">
                                &emsp;&emsp;<span class='cate_icon <?php if($su[sub] == false): ?>no_sub<?php else: ?>sub<?php endif; ?>'></span>
                                <span><?php echo ($su['title']); ?></span>
                            </div>
                            <div class="cate-action text-right">
                                <a href="<?php echo U('Node/insert',array('id'=>$su['id']));?>">
                                    编辑
                                </a>|
                                <a href="<?php echo U('Node/delete',array('id'=>$su['id']));?>">
                                    删除
                                </a>
                            </div>
                        </div>
                        <?php if(is_array($su[sub])): $i = 0; $__LIST__ = $su[sub];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ssu): $mod = ($i % 2 );++$i;?><div class="ssu display-none">
                                <div class="cate-item">
                                    <div class="cate-text">
                                        &emsp;&emsp;&emsp;&emsp;<span class="cate_icon no_sub"></span>
                                        <span><?php echo ($ssu['title']); ?></span>
                                    </div>
                                    <div class="cate-action text-right">
                                        <a href="<?php echo U('Node/insert',array('id'=>$ssu['id']));?>">
                                            编辑
                                        </a>|
                                        <a href="<?php echo U('Node/delete',array('id'=>$ssu['id']));?>">
                                            删除
                                        </a>
                                    </div>
                                </div>
